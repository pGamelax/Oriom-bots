    ...
    try {
      await signUp.email({
        ...
        fetchOptions: {
          ...
          onError: (ctx) => { toast.error(ctx.error.message ?? "Erro ao criar conta"); },
        },
      });
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-gradient-to-br from-lilac-light via-surface to-muted flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-muted-foreground mt-1">Comece a gerenciar seus bots</p>
        </div>
        ...
        <div className="bg-surface rounded-2xl border border-border p-6 shadow-xl shadow-primary/8">
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-medium text-text-medium">Nome</label>
              <input
                type="text"
                required
                value={name}
